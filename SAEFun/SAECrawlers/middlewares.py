...
class CustomDownloaderMiddleware(object):
    def process_response(self, request, response, spider):
        ...
        if len(response.url) > config.retriever_max_url_length:
            log.debug("%s # Url is too long" % response.url[:int(0.2 * config.retriever_max_url_length)])
            raise IgnoreRequest
        ...
        if (content_type is None) or (content_type not in config.retriever_allow_content_type):
            log.debug("%s # Unknown Content-Type:[%s] " % (response.url, content_type))
            raise IgnoreRequest

        for de in config.retriever_deny_domains:
            if response.url.find(de) != -1:
                log.debug("%s # Deny Domain" % (response.url,))
                raise IgnoreRequest
        ...
        if response.status == 404:  # not found
            if urldb_item is not None:  # already in DB
                log.debug("%s # Already in DB, but 404 now" % request.url)
                db.delete_sem_with_urlid(urldb_item['id'])
                raise IgnoreRequest
            else:  # new 404
                log.debug("%s # New 404 found" % response.url)
                raise IgnoreRequest
        ...
        if urldb_item is not None:
            ...
            if tool.hash_for_text(response.body) == urldb_item['content_hash']:
                ...
                log.debug("%s # Already in DB, Content has not change" % response.url)
                db.update_url_lastextractts(urldb_item['id'])
                raise IgnoreRequest
            else:
                log.debug("%s # Already in DB, Content changed" % response.url)
                # delete corresponding seminar info in SEM_LIB
                db.delete_sem_with_urlid(urldb_item['id'])
        else:
            log.debug(response.url + " # New url")
