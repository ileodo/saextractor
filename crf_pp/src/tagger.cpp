...
namespace CRFPP {
  ...
  bool TaggerImpl::add2(size_t size, const char **column, bool copy) {
    ...
    if ((mode_ == LEARN && size < xsize + 1) ||
        ...
        std::cerr << "ERR size=" << size << " xsize=" << xsize << "\n";
        CHECK_FALSE(false) << "# x is small: size=" << size << " xsize=" << xsize;
    }
  ...
  bool TaggerImpl::parse() {
    std::cerr << "building features\n";
    CHECK_FALSE(feature_index_->buildFeatures(this))
      << feature_index_->what();

    std::cerr << "checking x\n";
    if (x_.empty()) return true;
    std::cerr << "building lattice\n";
    buildLattice();
    if (nbest_ || vlevel_ >= 1) forwardbackward();
    std::cerr << "running viterbi\n";
    viterbi();
    if (nbest_) initNbest();
...
int crfpp_test(int argc, char **argv) {
  ...
  if (!tagger.open(&param)) {
    std::cerr << "ERR " << tagger.what() << std::endl;
    return -1;
  }

  std::string output = param.get<std::string>("output");
  if (output.empty()) output = "-";
  std::cerr << "Storing output to '"<<output.c_str()<<"'\n";
  CRFPP::ostream_wrapper os(output.c_str());
  if (!*os) {
    std::cerr << "ERR no such file or directory: " << output << std::endl;
    return -1;
  }
  ...
  for (size_t i = 0; i < rest.size(); ++i) {
    std::cerr << i<< ". processing input from '"<<rest[i].c_str()<<"'\n";
    CRFPP::istream_wrapper is(rest[i].c_str());
    if (!*is) {
      std::cerr << "ERR no such file or directory: " << rest[i] << std::endl;
      return -1;
    }
